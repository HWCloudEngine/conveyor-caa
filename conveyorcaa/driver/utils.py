...
from conveyorcaa.i18n import _, _LE, _LI, _LW
...
def get_root_helper():
    if CONF.workarounds.disable_rootwrap:
        ...
        cmd = 'sudo caa-rootwrap %s' % CONF.rootwrap_config
    return cmd
    ...
    def _get_client(cls, rootwrap_config):
        try:
            ...
            new_client = client.Client([
                "sudo", "caa-rootwrap-daemon", rootwrap_config])
...
def monkey_patch():
    ...
    Example:
    'caa.api.compute.ec2.cloud:compute.notifications.notify_decorator'
...
def sanitize_hostname(hostname, default_name=None):
    """Return a hostname which conforms to RFC-952 and RFC-1123 specs except
       ...
       Due to compute-network will leverage dnsmasq to set hostname, so we
       chose 63.

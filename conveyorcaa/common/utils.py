...
from conveyorcaa.common import processutils
...
def get_root_helper():
    pass
    # return 'sudo conveyorcaa-api %s' % CONF.rootwrap_config
...
def list_device():
    ...
    Example returns:
      [
        { "name": "/dev/sde", "type": "disk", "size": "3G",
        "maj:min": "8:16" },
        { "name": "/dev/sdh", "type": "disk", "size": "4G",
        "maj:min": "8:48" }
      ]
    """
    ...
    for dev in dev_out.strip().split('\n'):
        ...
        if disk_type == 'disk' and not name.endswith('da') and \
                DEVICE_RE.match(name):
            res[0] = "/dev/" + name
...
def get_available_byte(disk_path):
    try:
        # NOTE(nkapotoxin): execute the cmd eg.df --block--size=1 /image to
        # get the size of the available of the /image
        # above df (GNU coreutils) 8.12 the example result like
        # Filesystem        1B-blocks   Used    Available Use% Mounted on
        # udev           135390781440 348160 135390433280   1% /dev
        # we try to get the available on the string split[-3] 135390433280
        result = execute('df', '--block-size=1', disk_path)
        return int(result[0].split()[-3])
    except processutils.ProcessExecutionError as e:
        LOG.error("get_available_disksize error, cmd df --block-size=1 "
                  "%s" % disk_path, e)
        raise exception.DownloadError(e)
...
def get_unused_device(disk_bds):
    """ get the unused disk names
    :param: disk_bds eg.
      [
        { "name": "/dev/sde", "type": "disk", "size": "3G",
        "maj:min": "8:16" },
    ...
    :return: list of the unused disk_bds
    """

    unused_bds = []
    for disk_bd in disk_bds:
        dev_out, _err = trycmd('lsblk', '-n', '-o', 'FSTYPE', '-f',
                               disk_bd['name'])
        unused = True
        for dev in dev_out.strip().split('\n'):
            res = dev.split()
            if res:
                ...
                break
        if unused:
            unused_bds.append(disk_bd)

    return unused_bds


def extend_hc(image_file, block_device_name):
    """ extend the hypercontainer image to the block device
    :param: image_file eg. /image/abcd-bcad-ssde-ffxs
    :param: block_device_name eg. /dev/sdb

    """

    # extend image file
    LOG.debug("extend hc image: %s, bdn: %s", (image_file, block_device_name))
    dev_out, _err = trycmd('qemu-img-hw', 'convert', image_file,
                           block_device_name)
    if _err:
        # extend err, we should roll back
        LOG.error("extend error %s", _err)
        rb_out, rb_err = trycmd('dd', 'if=/dev/zero',
                                'of=%s' % block_device_name, 'count=10')
        trycmd('partprobe')
        if rb_err:
            LOG.error("extend rollback error, the data vol is broken")

        raise exception.ExtendError(image=image_file)


def scan_devices():
    """Some hypervisor do not support dynamical scan the device when the vm
    attached a new vol, we should do the following cmd:
    echo '- - -' > /sys/class/scsi_host/host*/scan

    """
    LOG.debug(_("Scaning host scsi devices"))
    trycmd("bash", "-c", "for f in /sys/class/scsi_host/host*/scan; "
                         "do echo '- - -' > $f; done")


def find_user_device():
    # try to find a user disk
    cmd = "lsblk -ndo 'NAME,TYPE' | awk " \
          "'$NF==\"disk\"&&$1~/^x?[a-z]?d?[a-z]$/{d=\"/dev/\"$1;" \
          "for(found=0;\"lsblk \"d|getline;)found+=$NF==\"/\";" \
          "if(found)next;print d;exit}'"
    dev_out, _err = trycmd("bash", "-c", cmd)
    for dev in dev_out.strip().split('\n'):
        res = dev.split()
        if res:
...
def mount_device(mount_path, device_link, root_device_link):
    """Find a new disk, mount it to the mount_path"""
    trycmd("mkdir", "-p", mount_path)
    user_device = find_user_device()[0]
    root_user_device = user_device + "1"
    try:
        trycmd("umount", mount_path)

        # mount the device
        execute("mount", root_user_device, mount_path)

        # record the device link
        LOG.info("Try to record the link device_link %(dl)s, "
                 "root_device_link %(rl)s",
                 {'dl': user_device, 'rl': root_user_device})
        trycmd("rm", "-rf", device_link, root_device_link)
        execute("ln", "-s", user_device, device_link)
        execute("ln", "-s", user_device + "1", root_device_link)
    except processutils.ProcessExecutionError as e:
        err_msg = "mount_device error, cmd mount %s" % (user_device +
                                                        mount_path)
        LOG.error(err_msg, e)
        trycmd("umount", mount_path)
        trycmd("rm", "-rf", device_link, root_device_link)
        raise exception.MountError(e)


def add_table_lookfor_domainname(endpoint, ip):
    protocol, host = urllib.splittype(endpoint)
    host, s = urllib.splithost(host)
    domainname, port = urllib.splitport(host)
    try:
        trycmd("sed", "-i", "/" + domainname + "/d", "/etc/hosts")

        # add new domianname
        execute("bash", "-c", "echo " + ip + "  " + domainname +
                " >> /etc/hosts")
    except processutils.ProcessExecutionError as e:
        LOG.error("Add static table for look up hostname error", e)
        raise exception.WormholeException(e)


def check_container_prepare_status(device_link):
    dev_out, _err = trycmd("bash", "-c", "[ -b $(readlink -f " +
                           device_link + ") ] || exit 1")
    if _err:
        LOG.error("Cannot find the prepared device")
        return False

    # find the container device prepared
    return True
